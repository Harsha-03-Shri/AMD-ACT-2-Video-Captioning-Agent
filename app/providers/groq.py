"""Groq provider for text-to-text caption generation."""
...
class GroqCaptionProvider(CaptionProvider):
    """Groq-hosted model for Stage 2: description → caption."""
    ...
    async def generate_caption(self, description: str, style: str) -> str:
        ...
        async def _generate() -> str:
            try:
                ...
                raise GenerationError(f"Groq caption timed out after {REQUEST_TIMEOUT}s")
            except GenerationError:
                raise
            except Exception as exc:
                raise GenerationError(f"Groq caption failed: {exc}") from exc
        ...
        try:
            ...
            raise GenerationError(f"Groq caption failed after retries: {exc}") from exc
